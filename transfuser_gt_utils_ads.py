...
def _draw_box_on_bev(
    ...
    # 转换为像素坐标
    pixel_coords = _coords_to_pixel(rotated_corners.reshape(-1, 1, 2), config)
    
    # 绘制填充多边形
    # 注意: OpenCV 使用 (width, height) 顺序
    bev_map_cv = np.ascontiguousarray(np.rot90(bev_map, k=-1))
    cv2.fillPoly(bev_map_cv, [pixel_coords], color=int(label))
    bev_map[:] = np.rot90(bev_map_cv)[::-1]
...
def _draw_static_objects(
    ...
    for i in range(static_feat.shape[0]):
        ...
        # 转换为像素坐标
        pixel_coords = _coords_to_pixel(valid_points.reshape(-1, 1, 2), config)
        
        # 绘制多边形线条
        bev_map_cv = np.ascontiguousarray(np.rot90(bev_map, k=-1))
        cv2.polylines(bev_map_cv, [pixel_coords], isClosed=False, color=int(label), thickness=2)
        bev_map[:] = np.rot90(bev_map_cv)[::-1]
...
def _draw_curb_lines(
    ...
    for i in range(curb_feat.shape[0]):
        ...
        # 绘制线条
        bev_map_cv = np.ascontiguousarray(np.rot90(bev_map, k=-1))
        cv2.polylines(bev_map_cv, [pixel_coords], isClosed=False, color=int(label), thickness=1)
        bev_map[:] = np.rot90(bev_map_cv)[::-1]
...
def visualize_bev_semantic_map(
    ...
    if agent_states is not None and agent_labels is not None:
        for i, (state, label) in enumerate(zip(agent_states, agent_labels)):
            ...
            # 转换到像素坐标
            px = int(W / 2 - y / config.bev_pixel_size)
            py = int(H - 1 - x / config.bev_pixel_size)
            
            # 绘制旋转矩形
            half_l = length / config.bev_pixel_size / 2
            half_w = width / config.bev_pixel_size / 2
            ...
            rotated[:, 0] = corners[:, 0] * cos_h - corners[:, 1] * sin_h + px
            rotated[:, 1] = corners[:, 0] * sin_h + corners[:, 1] * cos_h + py
